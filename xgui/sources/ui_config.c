...

{
    ...
    err = fprintf(filePtr,"SNNS xgui configuration file.  format: %d\n\n", CURRENT_FORMAT); retchk(err);

    err = fprintf(filePtr,"filenames:\n"); retchk(err);
    
    err = fprintf(filePtr,"%s#\n",ui_pathname); retchk(err);
    err = fprintf(filePtr,"%s#\n",ui_filenameNET); retchk(err);
    err = fprintf(filePtr,"%s#\n",ui_filenamePAT); retchk(err);
    err = fprintf(filePtr,"%s#\n",ui_filenameRES); retchk(err);
    err = fprintf(filePtr,"%s#\n",ui_filenameCFG); retchk(err);
    err = fprintf(filePtr,"%s#\n",ui_filenameTXT); retchk(err);
    ...
    {
	err = fprintf(filePtr,"%s#\n",&ui_layerNames[i-1][0]); retchk(err);
    }
    err = fprintf(filePtr,"\n"); retchk(err);
    ...
    /*
     * DANYSAN
     * PROBLEMA LOGICO SOBRE CASCADE CUANDO NO HAY CASCADE
     * Solucionado. 27-10-23
     */
	if( cc_open )
	{
		cc_readElements();
		...
		{
			err = fprintf(filePtr,"param %d: %9.5f\n",i+1, ui_learnParameters[i]); retchk(err);
		}
	}
// ---------- SOLUCIONAR ------- ESTO GENERA 25 PARAMETERS FALSOS.
	...
	{
		if (ui_controlIsCreated) 
	   		err = fprintf(filePtr,"param %d: %9.5f\n",i+1, ui_xFloatFromAsciiWidget(ui_updateParameterWidgets[i]));
		else
			err = fprintf(filePtr,"param %d: %9.5f\n",i+1, ui_updateParameters[i]);
	retchk(err);
    }
    err = fprintf(filePtr,"\n"); retchk(err);
    ...
    for (i=0; i<in; i++) {
	if (ui_controlIsCreated) 
	    err = fprintf(filePtr,"param %d: %9.5f\n",i+1, ui_xFloatFromAsciiWidget(ui_initParameterWidgets[i]));
	else
	    err = fprintf(filePtr,"param %d: %9.5f\n",i+1, ui_initParameters[i]);
	retchk(err);
    }

    krui_GetPatInfo(&patt_info, &descrip);
    err = fprintf(filePtr,"Remap parameters: %d\n", patt_info.no_of_remap_params); retchk(err);
    for (i=0; i<patt_info.no_of_remap_params; i++) {
	if (ui_controlIsCreated) 
	    err = fprintf(filePtr,"param %d: %9.5f\n",i+1, ui_xFloatFromAsciiWidget(ui_remapParameterWidgets[i]));
	else
	    err = fprintf(filePtr,"param %d: %9.5f\n",i+1, ui_remapParameters[i]);
	retchk(err);
    }
    err = fprintf(filePtr,"\n"); retchk(err);
    ...
    err = fprintf(filePtr,"Displays: %d\n\n",count);  retchk(err); /* no of displays */

    listPtr = ui_displ_listPtr;
    while (listPtr != NULL) {
	err = fprintf(filePtr,"display no             : %d\n",listPtr->displayNo); retchk(err);
	err = fprintf(filePtr,"position x             : %d\n",(int) listPtr->x); retchk(err);
	err = fprintf(filePtr,"position y             : %d\n",(int) listPtr->y); retchk(err);
	err = fprintf(filePtr,"width                  : %d\n",(int) listPtr->width); retchk(err);
	err = fprintf(filePtr,"height                 : %d\n",(int) listPtr->height); retchk(err);
	err = fprintf(filePtr,"origin x               : %d\n",(int) listPtr->origin.x); retchk(err);
	err = fprintf(filePtr,"origin y               : %d\n",(int) listPtr->origin.y); retchk(err);
	err = fprintf(filePtr,"gridSize               : %d\n",listPtr->gridSize); retchk(err);
	err = fprintf(filePtr,"frozen                 : %d\n",listPtr->frozen); retchk(err);
	err = fprintf(filePtr,"raster                 : %d\n",listPtr->raster); retchk(err);
	err = fprintf(filePtr,"subNetNo               : %d\n",listPtr->subNetNo); retchk(err);
	err = fprintf(filePtr,"flags                  : %d\n",listPtr->flags); retchk(err);
	err = fprintf(filePtr,"layers                 : %d\n",(int) listPtr->layers); retchk(err);
	err = fprintf(filePtr,"update type            : %d\n",listPtr->updateType); retchk(err);
	err = fprintf(filePtr,"setup show title       : %d\n",listPtr->setup.showTitleFlg); retchk(err);
	err = fprintf(filePtr,"setup title            : %d\n",listPtr->setup.showTitle); retchk(err);
	err = fprintf(filePtr,"setup show value       : %d\n",listPtr->setup.showValueFlg); retchk(err);
	err = fprintf(filePtr,"setup value            : %d\n",listPtr->setup.showValue); retchk(err);
	err = fprintf(filePtr,"setup show link        : %d\n",listPtr->setup.showLinkFlg); retchk(err);
	err = fprintf(filePtr,"setup show direction   : %d\n",listPtr->setup.showDirectionFlg); retchk(err);
	err = fprintf(filePtr,"setup show weight      : %d\n",listPtr->setup.showWeightFlg); retchk(err);
	err = fprintf(filePtr,"setup link trigger pos : %9.5f\n",(float) listPtr->setup.linkPosTrigger); retchk(err);
	err = fprintf(filePtr,"setup link trigger neg : %9.5f\n",(float) listPtr->setup.linkNegTrigger); retchk(err);
	err = fprintf(filePtr,"setup unit scale factor: %8.5f\n",(float) listPtr->setup.unitScaleFactor); retchk(err);
	err = fprintf(filePtr,"setup link scale factor: %8.5f\n",(float) listPtr->setup.linkScaleFactor); retchk(err);
	err = fprintf(filePtr,"setup site name        : %s#\n", listPtr->setup.siteName); retchk(err);
	err = fprintf(filePtr,"setup display colors   : %02d%02d%02d\n", listPtr->setup.textColorIndex, listPtr->setup.backgroundColorIndex, listPtr->setup.selectionColorIndex); retchk(err);
	err = fprintf(filePtr,"\n"); retchk(err);
	listPtr = listPtr->nextPtr;
    }
	...
	{
        err = fprintf (filePtr, "%s\n", d3_configStr); retchk(err);
        err = fprintf(filePtr,"\n"); retchk(err);
        err = fprintf (filePtr, "initial rotation               : %9.4f %9.4f %9.4f\n", deg(d3_state.rot_vec[0]), deg(d3_state.rot_vec[1]), deg(d3_state.rot_vec[2])); retchk(err);
        err = fprintf (filePtr, "initial translation            : %9.4f %9.4f %9.4f\n", d3_state.trans_vec[0], d3_state.trans_vec[1], d3_state.trans_vec[2]); retchk(err);
        err = fprintf (filePtr, "initial scale                  : %9.4f %9.4f %9.4f\n", d3_state.scale_vec[0], d3_state.scale_vec[1], d3_state.scale_vec[2]); retchk(err);
        err = fprintf (filePtr, "rotation step                  : %9.4f %9.4f %9.4f\n", deg(d3_state.rot_step[0]), deg(d3_state.rot_step[1]), deg(d3_state.rot_step[2])); retchk(err);
        err = fprintf (filePtr, "translation step               : %9.4f %9.4f %9.4f\n", d3_state.trans_step[0], d3_state.trans_step[1], d3_state.trans_step[2]); retchk(err);
        err = fprintf (filePtr, "scale step                     : %9.4f %9.4f %9.4f\n", d3_state.scale_step[0], d3_state.scale_step[1], d3_state.scale_step[2]); retchk(err);
        err = fprintf (filePtr, "projection                     : %d\n", d3_state.projection_mode); retchk(err);
        err = fprintf (filePtr, "viewpoint                      : %9.4f %9.4f %9.4f\n", d3_state.viewpoint[0], d3_state.viewpoint[1], d3_state.viewpoint[2]); retchk(err);
        err = fprintf (filePtr, "font                          : %d\n", d3_state.font); retchk(err);
        err = fprintf (filePtr, "model mode                     : %d\n", d3_state.model_mode); retchk(err);
        err = fprintf (filePtr, "link mode                      : %d\n", d3_state.link_mode); retchk(err);
        err = fprintf (filePtr, "link scale                     : %9.4f\n", d3_state.link_scale); retchk(err);
        err = fprintf (filePtr, "unit aspect                    : %9.4f\n", d3_state.unit_aspect); retchk(err);
        err = fprintf (filePtr, "show unit size                 : %d\n", d3_state.unit_mode.size); retchk(err);
        err = fprintf (filePtr, "show unit color                : %d\n", d3_state.unit_mode.color); retchk(err);
        err = fprintf (filePtr, "show unit top label            : %d\n", d3_state.unit_mode.top_label); retchk(err);
        err = fprintf (filePtr, "show unit bottom label         : %d\n", d3_state.unit_mode.bottom_label); retchk(err);
        err = fprintf (filePtr, "light position                 : %9.4f %9.4f %9.4f\n", d3_state.light.position[0], d3_state.light.position[1], d3_state.light.position[2]); retchk(err);
        err = fprintf (filePtr, "ambient light intensity        : %9.4f\n", d3_state.light.Ia); retchk(err);
        err = fprintf (filePtr, "ambient reflection coefficient : %9.4f\n", d3_state.light.Ka); retchk(err);
        err = fprintf (filePtr, "point light intensity          : %9.4f\n", d3_state.light.Ip); retchk(err);
        err = fprintf (filePtr, "diffuse refection coefficient  : %9.4f\n", d3_state.light.Kd); retchk(err);
        err = fprintf(filePtr,"\n"); retchk(err);
   }  

   err = fprintf (filePtr, "%s\n\n", ui_printerConfigStr); retchk(err);
    
   err = fprintf (filePtr, "destination               : %d\n", ui_prVal.dest); retchk(err);
   err = fprintf (filePtr, "format                    : %d\n", ui_prVal.format); retchk(err);
   err = fprintf (filePtr, "paper                     : %d\n", ui_prVal.paper); retchk(err);
   err = fprintf (filePtr, "auto scale                : %d\n", ui_prVal.autoScale); retchk(err);
   err = fprintf (filePtr, "clip on border            : %d\n", ui_prVal.clip); retchk(err);
   err = fprintf (filePtr, "color                     : %d\n", ui_prVal.color); retchk(err);
   err = fprintf (filePtr, "orientation               : %d\n", ui_prVal.orient); retchk(err);
   err = fprintf (filePtr, "scale uniform             : %d\n", ui_prVal.autoUni); retchk(err);
   err = fprintf (filePtr, "unitsize by value         : %d\n", ui_prVal.size); retchk(err);
   err = fprintf (filePtr, "unitshape                 : %d\n", ui_prVal.shape); retchk(err);
   err = fprintf (filePtr, "transparent text          : %d\n", ui_prVal.text); retchk(err);
   err = fprintf (filePtr, "border                    : %d\n", ui_prVal.border); retchk(err);
   err = fprintf (filePtr, "resolution                : %d\n", ui_prVal.resolution); retchk(err);
   err = fprintf (filePtr, "display to print          : %d\n", ui_prVal.displayToPrint); retchk(err);
   err = fprintf (filePtr, "horizontal border         : %.2f\n", ui_prVal.borderHoriz); retchk(err);
   err = fprintf (filePtr, "vertical border           : %.2f\n", ui_prVal.borderVert); retchk(err);
   err = fprintf (filePtr, "image x scale             : %.2f\n", ui_prVal.scaleValX); retchk(err);
   err = fprintf (filePtr, "image y scale             : %.2f\n", ui_prVal.scaleValY); retchk(err);
   err = fprintf (filePtr, "unit gray value           : %.2f\n", ui_prVal.unitGray); retchk(err);
   err = fprintf (filePtr, "file name                 : %s\n", ui_prVal.fileNameStr); retchk(err);
   err = fprintf (filePtr, "command line              : %s\n", ui_prVal.cmdLineStr); retchk(err);

   err = fprintf (filePtr, "\n%s\n\n", ui_classConfigStr); retchk(err);
   if(ui_class_open)
		   class_readWidgets();
   err = fprintf (filePtr, "%d %d ",ui_classes,patt_info.class_distrib_active); retchk(err);
   for(i=0; i<ui_classes; i++)
   {
       err = fprintf(filePtr,"%d ", *patt_info.class_redistribution++);
   }
   err = fprintf(filePtr,"\n"); retchk(err);
   err = fprintf (filePtr, "\n%s\n\n", ui_windowConfigStr); retchk(err);
   err = fprintf (filePtr, "%d %d %d %d %d \n", ui_controlIsCreated, ui_infoIsCreated, ui_class_open, pr_open, cc_open); retchk(err);
   err = fprintf(filePtr,"\n"); retchk(err);
