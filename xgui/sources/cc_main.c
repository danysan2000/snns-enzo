...
void cc_readElements(void)
{
    ...
    if((cc_learningFunc == -1) || (cc_candActFunc == -1) || (cc_pruneFunc == -1))
	{
...
void cc_createWindow (void)
{
    ...
    if(!cc_open)
	{
		...
		n = 0;  
		cc_mainWidget = XtCreatePopupShell(buf,topLevelShellWidgetClass, ui_toplevel,arg,n); 
		...
		XtAddEventHandler(box,KeyPressMask,FALSE, (XtEventHandler)ui_key_control,(Cardinal *) 0);
