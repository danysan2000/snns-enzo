/*****************************************************************************
  ...
  LAST CHANGE    : $Date: Wed 11 Oct 2023 05:55:32 PM -03
...
{
    XDrawLine(displ, d, gc, pixPos1.x, pixPos1.y, pixPos2.x, pixPos2.y);
}
...
void ui_xDrawTriangle(Display *displ, Drawable d, GC gc, struct PosType pixPos1, struct PosType pixPos2)
{
    ...
    for (i=1; i<3; i++)
    {   /* graphic transformation: rotation angle */
    ...
    /* draw arrow */
    /* pw_polygon_2(ui_pw, pixPos2.x, pixPos2.y, nbds, npts, vlist_triangle, raster_op, NULL, 0,0); */
    /* for (i=0; i<3; i++)
		XDrawLine(displ, d, gc, (vlist_triangle[i].x + pixPos2.x), (vlist_triangle[i].y + pixPos2.y), (vlist_triangle[(i+1) MOD 3].x + pixPos2.x), (vlist_triangle[(i+1) MOD 3].y + pixPos2.y) );
    */
    for (i=1; i<3; i++)
	XDrawLine(displ, d, gc, (vlist_triangle[0].x + pixPos2.x), (vlist_triangle[0].y + pixPos2.y), (vlist_triangle[i].x + pixPos2.x), (vlist_triangle[i].y + pixPos2.y) );
}
...

{
    ...
    XChangeWindowAttributes(display, window, CWBackingStore | CWSaveUnder, &wa);
#endif       
}
